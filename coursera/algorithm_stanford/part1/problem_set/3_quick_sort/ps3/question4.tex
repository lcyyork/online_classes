...
\newcommand{\hmwkTitle}{Problem Set 3: Question \#4} % Assignment title
...
\vspace{10pt}
\problemAnswer{
Let $Y$ = number of pairs of people with the same birthday.
The goal is to find $k$ such that $E[Y] \geq 1$.
\begin{align}
  X_i = 
  \begin{cases}
    1 \quad \text{if a pair of people has the same birthday.} \\
    0 \quad \text{otherwise.}
  \end{cases}
\end{align}
The probability for a pair of people has the same birthday is $P[X_i] = 1/365$.
There are $N = \binom{k}{2} = \frac{k(k-1)}{2}$ such pairs.
Thus, the expectation of $Y$ is:
\begin{align}
    E[Y] &= E[\sum_{i=1}^{N} X_i] = \sum_{i=1}^{N} E[X_i] = \sum_{i=1}^{N} P[X_i] \notag\\
    &= \frac{N}{365} = \frac{k(k-1)}{2 \times 365}.
\end{align}
If we let $E[Y] \geq 1$ and solve for $k$, we find the smallest $k$ is 28.
}
